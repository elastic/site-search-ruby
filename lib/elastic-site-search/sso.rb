...
module ElasticSiteSearch
  ...
  module SSO
    ...
    def self.url(user_id)
      ...
      "#{BASE_URL}?user_id=#{user_id}&client_id=#{ElasticSiteSearch.platform_client_id}&timestamp=#{timestamp}&token=#{token(user_id, timestamp)}"
    end

    def self.token(user_id, timestamp)
      Digest::SHA1.hexdigest("#{user_id}:#{ElasticSiteSearch.platform_client_secret}:#{timestamp}")
    end
  end
end
